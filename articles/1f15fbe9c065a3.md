...
合計登録前で33000件なのは、HTTPリクエストのエラー後にリトライ機構が働いているからだ。
改善後でも13916件なのは、3,000VU × 50フィード = 150,000はあくまでテストの目標設定なので、ランプアップ（VUの段階的起動 60秒 + 試験本番 120秒 + ramp-down 30秒 ≒ 187秒 の枠内で回せる数には限りがあるため、150000件はいずれにせよ捌けていない。

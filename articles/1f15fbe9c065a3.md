...
周囲からもらったフィードバックを元に、今回から切り口を変えていく Part 3。
...
記事では、改善後に同等の条件で計測した後に、さらに高負荷でも計測してパフォーマンス向上が見られたので比較がズレるのを承知で、改善後の条件で比較していく。
...
合計登録数で33000件なのは、HTTPリクエストのエラー後にリトライ機構が働いているからだ。
改善後でも13916件なのは、3,000VU × 50フィード = 150,000はあくまでテストの目標設定なので、

- x. ランプアップ（VUの段階的起動） 60秒
- y. 試験本番 120秒
- z. ramp-down 30秒

x + y + z ≒ 187秒 の枠内で回せる数には限りがあるため150000件はいずれにせよ捌けていない。
これを改善するには、Mockサーバーのスケーリングや、別マシンから通信するなど、負荷試験自体を改善する必要があるだろう。

![Altの負荷試験前後のフィード登録結果](/images/alt/alt-backend/pgb_07_registration_result.webp)

（40.2% = 成功イテレーション 13,378 ÷ 総イテレーション 33,306）


## RSSフィードの登録機能について

- Handler、Domain、Usecase、Port、Gateway、Driverの層を備えた、クリーンアーキテクチャを採用したバックエンドAPIがメインでリクエストをさばく。
...
以降は特筆がなければ、PgBouncer導入（改善）前の結果だ。

- HTTPリクエスト失敗率は17.27%で、目標の20%を下回った。
- フィード登録率は81.28%で、目標の80%を上回った。
  - これはVUのイテレーションの成功率で、全リクエストの成功率ではないことに注意が必要だ。
  - 他のメトリクスも照らし合わせると、フィード登録機能自体、つまりアプリケーション側の実装には問題が無いと解釈できる。
- フィード登録失敗が多数発生していることは大きな問題だ。
  - これはフィードの登録リクエスト自体が失敗していることを示すもので、リクエストは多数失敗していることがわかる。

<br>

> 条件: 3,000VU × 50フィード / 120秒 / K6リトライ実装修正前
...
少なくとも自分とLLMで調査した中では1ユーザーや少数ユーザーレベルでは、アーキテクチャや設計に問題はなかった。それは1250 ops/secを捌くような設計にはなっていないからだ。
だが良い機会なので、[PgBouncer](https://pgbouncer.org/)を導入してみた。
システムが多数のユーザーに使われてリクエストのスパイクが発生した時でも安定的に稼働するシステムへと改修した。
...
- PgBouncer（transaction poolingモード）
- `alt-db` および `kratos-db` の前段に導入
- `alt-db`には、DEFAULT_POOL_SIZE=30、MAX_DB_CONNECTIONS=100を設定
...
K6のリトライ実装を修正後に改善前を再計測したところ、`http_req_failed`は47.10%となった。
前節のK6初回実装時の17.27%より悪化しているのは、リトライが無くなりエラーが隠れなくなったためだ。

> 条件: 3,000VU × 100フィード / 120秒 / K6リトライ実装修正済み
...
※補足だが、`feed_register_errors count`の変化は、負荷試験のK6側での実装を改善後に、PgBouncer導入以前以後で再計測したものだ。

`feed_register_duration`の微増については、リクエストのエラーが減り処理されるフィードが増えたことにあるものだ。つまり、システムが正常に動作していることを示しているといえる。
...
[次回: RSSリーダー・アグリゲーターの作り方 Part 4: スループット8.46倍向上とその裏側](https://zenn.dev/e_kaikei/articles/ad129f6fb2ed39)

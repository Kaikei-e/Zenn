...
flowchart TD
    U["Execute(ctx, link)"]
    U --> S1["1. URL検証 + RSSフェッチ"]
    S1 --> S2["2. feed_link DB登録"]
    S2 --> S3["3. 同一URLへ再フェッチ（冗長）"]
    S3 --> S4["4. パース → feed items取得"]
    S4 --> S5["5. feed items DB登録"]
```
...
flowchart TD
    U["Execute(ctx, link)"]
    U --> S1["1. URL検証 + RSSフェッチ（1回）"]
    S1 --> S2["2. feed_link DB登録"]
    S2 --> S3["3. feed_link_id取得"]
    S3 --> S4["4. FeedLinkIDセット"]
    S4 --> S5["5. feed items DB登録"]
    S5 --> S6["6. 非同期: イベント発行 + 自動購読"]
```

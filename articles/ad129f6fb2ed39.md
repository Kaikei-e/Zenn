...
flowchart TD
    U["Execute(ctx, link)"]
    U --> S1["1. Validate + Fetch RSS"]
    S1 --> S2["2. INSERT feed_links"]
    S2 --> S3["3. Re-fetch same URL (redundant)"]
    S3 --> S4["4. Parse RSS → feed items"]
    S4 --> S5["5. INSERT feeds"]
```
...
flowchart TD
    U["Execute(ctx, link)"]
    U --> S1["1. Validate + Fetch RSS (single HTTP call)"]
    S1 --> S2["2. INSERT feed_links"]
    S2 --> S3["3. Resolve feed_link_id"]
    S3 --> S4["4. Set FeedLinkID on items"]
    S4 --> S5["5. INSERT feeds"]
    S5 --> S6["6. Fire-and-forget: publish events + auto-subscribe"]
```
